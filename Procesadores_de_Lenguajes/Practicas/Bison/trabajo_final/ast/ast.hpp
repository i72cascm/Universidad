 ...
 class ExpNode 
{
  public:
	...
	*/
	...
	/*!	
	\brief   Evaluate the expression as ALPHA
	\warning Virtual function: could be redefined in the heir classes
	\return  string
	\sa		 print
	*/
    virtual std::string evaluateAlpha()
	{
		return "";
	}
...
class VariableNode : public ExpNode 
{
	...
	*/
	...
	/*!	
	\brief   Evaluate the Variable as ALPHA
	\return  string
	\sa		 printAST
	*/
    std::string evaluateAlpha();
...
/*!	
  \class   AlphaOperatorNode
  \brief   Definition of atributes and methods of AlphaOperatorNode class
  \note    AlphaOperatorNode Class publicly inherits from OperatorNode class
  \warning Abstract class, because it does not redefine the printAST method of ExpNode
*/
class AlphaOperatorNode : public OperatorNode 
{
	public:

	/*!		
		\brief Constructor of AlphalOperatorNode uses OperatorNode's constructor as members initializer
		\param L: pointer to ExpNode
		\param R: pointer to ExpNode
		\post  A new AlphaOperatorNode is created with the parameters
	*/
    AlphaOperatorNode(ExpNode *L, ExpNode *R): OperatorNode(L,R) 
	{
		//	Empty
	}
...
/*!	
  \class AlphaNode
  \brief Definition of atributes and methods of AlphaNode class
  \note  AlphaNode Class publicly inherits from ExpNode class
*/
class AlphaNode : public ExpNode 
{
 private:
   std::string _string; //!< \brief string of the AlphaNode
 
 public:

/*!		
	\brief Constructor of AlphaNode
	\param value: string
	\post  A new AlphaNode is created with the value of the parameter
	\note  Inline function
*/
  AlphaNode(std::string value)
	{
	    this->_string = value;
	}

	/*!	
	\brief   Get the type of the expression: ALPHA
	\return  int
	\sa		 printAST
	*/
	int getType();
...
/*!	
  ...
  \note    ConcatNode Class inherits from AlphaOperatorNode class 
*/
class ConcatNode : public AlphaOperatorNode 
{
  public:
/*!		
	\brief Constructor of ConcatNode
	\param L: pointer to left
	\param R: pointer to right
	\post  A new ConcatNode is created with the parameter
*/
  ConcatNode(ExpNode *L,ExpNode *R) : AlphaOperatorNode(L,R){}
/*!
...
/*!	
  \class   DivIntNode
  \brief   Definition of atributes and methods of DivIntNode class
  \note    DivIntNode Class publicly inherits from NumericOperatorNode class 
		   and adds its own print and evaluate functions
*/
class DivIntNode : public NumericOperatorNode 
{
  public:
/*!		
	\brief Constructor of DivIntNode uses NumericOperatorNode's constructor as members initializer
	\param L: pointer to ExpNode
	\param R: pointer to ExpNode
	\post  A new DivIntNode is created with the parameter
*/
  DivIntNode(ExpNode *L, ExpNode *R): NumericOperatorNode(L,R) 
  {
		// Empty
  }
/*!
	\brief   Print the DivIntNode
	\return  void
	\sa		 evaluateNumber
*/
  void printAST();
...
/*!	
  \class   PrintStmtString
  \brief   Definition of atributes and methods of PrintStmtString class
  \note    PrintStmtString Class publicly inherits from Statement class 
		   and adds its own print and evaluate functions
  \warning  In this class, printAST and evaluate functions have the same meaning.
*/
class PrintStmtString: public Statement 
{
 private:
  ExpNode *_exp; //!< Expresssion the print statement
 ...
 public:
/*!		
	\brief Constructor of PrintStmtString 
	\param expression: pointer to ExpNode
	\post  A new PrintStmtString is created with the parameter
*/
  PrintStmtString(ExpNode *expression)
	{
		this->_exp = expression;
	}

/*!
	\brief   Print the AST for PrintStmtString
	\return  void
	\sa		   evaluate
*/
  void printAST();
...
/*!	
	\brief   Evaluate the PrintStmtString
	\return  double
	\sa		   printAST
*/
  void evaluate();
...
/*!	
  \class   ClearStmt
  \brief   Definition of ClearStmt class
  \note    ClearStmt Class pinherits from Statement class 
*/
class ClearStmt : public Statement{
  // No attributes
  public:
/*!		
	\brief Constructor ClearStmt
...
/*!	
  \class   PlaceStmt
  \brief   Definition of PlaceStmt class
  \note    PlaceStmt inherits from Statement class 
...
/*!	
  \class   ReadStmtString
  \brief   Definition ReadStmtString class
  \note    ReadStmtString Class inherits from Statement class 

*/
class ReadStmtString : public Statement{
  private:
	std::string _id; //!< Name of the ReadStmtString
	
  public:
/*!		
	\brief Constructor ReadStmtString
	\param id: string
*/
  ReadStmtString(std::string id){
		this->_id = id;
	}
/*!
	\brief   Print AST for ReadStmtString
	\return  void
*/
  void printAST();

/*!	
	\brief   Evaluate ReadStmtString
	\return  void
*/
  void evaluate();
};
...
class IfStmt : public Statement{
 private:
  ...
  std::list<Statement *> *_stmts1; 
  std::list<Statement *> *_stmts2;

  public:
/*!		
	\brief Constructor of Single IfStmt (without alternative)
	\param condition: ExpNode of the condition
	\param stmtList: Statements of the consequent
	\post  A new IfStmt is created with the parameters
*/
  IfStmt(ExpNode *condition, std::list<Statement *> *stmtList){
		this->_cond = condition;
		this->_stmts1 = stmtList;
		this->_stmts2 = NULL;
	}
/*!		
	\brief Constructor of Compound IfStmt (with alternative)
	\param condition: ExpNode of the condition
	\param stmtList1: Statements of the consequent
	\param stmtList2: Statements of the alternative
	\post  A new IfStmt is created with the parameters
*/
...
/*!	
  ...
  \note    ForStmt inherits from Statement class 
*/
class ForStmt : public Statement{
 private:
  ExpNode *_from; //!<  first value
  ExpNode *_until; //!< last value
  ExpNode *_step; //!< iteration
  std::string _identifier; //!< identifier
  std::list<Statement *> *_stmts;  //!<statements inside the loop

  public:
/*!		
	\brief Constructor of  ForStmt
	\param identifier: condition
	\param from: first value
	\param until: last value
	\param step: iteration
	\param stmtList: Statements of the body of the for loop 
*/
  ForStmt(std::string identifier,ExpNode *from,ExpNode *until,ExpNode *step,std::list<Statement *> *stmtList): _identifier(identifier){
		this->_from = from;
		this->_until = until;
		this->_step = step;
		this->_stmts = stmtList;		
	}
  ForStmt(std::string identifier,ExpNode *from,ExpNode *until,std::list<Statement *> *stmtList): _identifier(identifier){
		this->_from = from;
		this->_until = until;
...
/*!	
  \class   RepeatStmt
  \brief   Definition of RepeatStmt class
  \note    RepeatStmt inherits from Statement class 
*/
class RepeatStmt : public Statement 
{
 private:
  ExpNode *_cond; //!< condition
  std::list<Statement *> *_statements;  //!< body of the loop
...
*/
  RepeatStmt (ExpNode *cond, std::list<Statement *> *statements){
		this->_cond = cond;
...
class Value {
 public:
  ExpNode *_value; //!< Value of the case
  std::list<Statement *> *_stmts;  //!< statements
/*!		
	\brief Constructor Value
  	\param value: pointer to value 
	\param stmtList: pointer to Statements

*/
  Value(ExpNode *value,std::list<Statement *> *stmtList){
		this->_value = value;
    	this->_stmts = stmtList;
	}
/*!
...
/*!	
  \class   CaseStmt
  \brief   Definition of CaseStmt class
  \note    CaseStmt Class inherits from Statement class 
...
class CaseStmt : public Statement{
	...
	private:
		ExpNode * _exp; //!<  Initial val statement
		std::list<Value *> *_vals; //!<  Possible values of the switch
		std::list<Statement *> *_default; //!<  Statement list of default

	public:
	/*!		
	\brief Constructor of  CaseStmt
	\param expression: Expression in case
	\param listOfValues: Possible values of the case
	\param defaultStmts: List of statements in default
	\post  New CaseStmt created with the parameters
*/
	CaseStmt(ExpNode *exp, std::list<Value *> *vals, std::list<Statement *> *defaultVal){
		this->_exp = exp;
		this->_vals = vals;
