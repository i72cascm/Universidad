...
class StringOperatorNode : public OperatorNode 
{
	...
	/*!		
		\brief Constructor of StringlOperatorNode uses OperatorNode's constructor as members initializer
		\param L: pointer to ExpNode
		\param R: pointer to ExpNode
		\post  A new NumericOperatorNode is created with the parameters
	*/
...
/*!	
  \class   AskForKeyStmt
  \brief   Definition of atributes and methods of AskForKeyStmt class
  \note    AskForKeyStmt Class publicly inherits from Statement class 
		   and adds its own print and evaluate functions
  \warning  In this class, print and evaluate functions have the same meaning.
*/
class AskForKeyStmt: public Statement 
{
 private:
  ExpNode *_msg; //!< Message shown on terminal

 public:
/*!		
	\brief Constructor of AskForKeyStmt 
	\param msg : Message that will be showed in terminal
	\post  A new AskForKeyStmt is created with the message
*/
  AskForKeyStmt(ExpNode *msg)
	{
		this->_msg = msg;
	}

/*!
	\brief   Print the AskForKeyStmt
	\return  void
	\sa		 evaluate()
*/
  void print();
...
/*!	
	\brief   Evaluate the AskForKeyStmt
	\return  double
	\sa		 print
*/
  void evaluate();
...
/*!	
  ...
  \brief   Definition of atributes and methods of ReadStmtString class
  \note    ReadStmtString Class publicly inherits from Statement class 
		   and adds its own print and evaluate functions
*/
class ReadStmtString : public Statement 
{
  ...
  public:
/*!		
	\brief Constructor of ReadStmtString
	\param id: string, name of the variable of the ReadStmtString
	\post  A new ReadStmtString is created with the parameter
*/
...
/*!
	\brief   Print the ReadStmtString
	\return  void
	\sa		 evaluate()
*/
  void print();
...
/*!	
	\brief   Evaluate the ReadStmtString
	\return  void
	\sa		 print
*/
  void evaluate();
...
class DoWhileStmt : public Statement 
{
 private:
  ExpNode *_cond; //!< Condicion of the while statement
  std::list<Statement *> *_stmts;  //!< List of statements of the body of the do while loop

  public:
/*!		
	\brief Constructor of  DoWhileStmt
	\param condition: ExpNode of the condition
	\param stmtList: Statements of the body of the loop 
	\post  A new DoWhileStmt is created with the parameters
*/
  DoWhileStmt(ExpNode *condition, std::list<Statement *> *stmtList)
	{
		this->_cond = condition;
		this->_stmts = stmtList;
	}
...
{
 private:
  ExpNode *_from; //!<  initial value of the for statement
  ExpNode *_until; //!< last value of the for statement
  ExpNode *_step; //!< value by which the identifier is incremented
  std::string _identifier; //!< identifier of the for statement
  std::list<Statement *> *_stmts;  //!< List of statements of the body of the for loop

  public:
/*!		
	\brief Constructor of  ForStmt
	\param identifier: ExpNode of the condition
	\param from: Value to which the identifier is inialized
	\param until: Value that finishes the loop once the identifier has reached it
	\param step: Value that modifies the identifier at the end of each iteration
...
class Value {
 public:
  ExpNode *_value; //!< Value required to run the statements of the case
  std::list<Statement *> *_stmts;  //!< List of statements

/*!		
	\brief Constructor of Value
  \param value: pointer to value of the case
	\param stmtList: pointer to a list of pointers to Statement
	\post  A new Value is created with its parameters
*/
...
{
 private:
  ExpNode *_expression; //!<  initial value of the for statement
  std::list<Value *> *_values; //!<  List of possible values of the switch
  std::list<Statement *> *_defaultStmts;  //!< List of statements of the default case of the switch

  public:
/*!		
	\brief Constructor of  SwitchStmt
	\param expression: Expression used in switch
	\param listOfValues: List of possible values of the switch
	\param defaultStmts: List of statements of the default case of the switch
	\post  A new SwitchStmt is created with the parameters
*/
  SwitchStmt(ExpNode *expression, std::list<Value *> *listOfValues, std::list<Statement *> *defaultStmts)
	{
		this->_expression = expression;
		this->_values = listOfValues;
		this->_defaultStmts = defaultStmts;
	}
...
{
  private:
	ExpNode *_n1; //!<  X position to place the cursor
	ExpNode *_n2; //!<  Y position to place the cursor
...
*/
  PlaceStmt(ExpNode *n1, ExpNode *n2)
	{
		this->_n1 = n1;
		this->_n2 = n2;
	}
