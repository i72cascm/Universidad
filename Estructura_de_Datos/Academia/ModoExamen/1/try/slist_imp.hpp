...
void SList<T>::push_front(T const &new_it)
{
    ...
    if(is_empty()){
        ...
        auto newA = SNode<T>::create(new_it, _head);
        _head = newA;
        _cursor = _head;
    }
    _size++;
...
void SList<T>::insert(T const &new_it)
{
    ...
    if(is_empty()){
        ...
        auto newA = SNode<T>::create(new_it, _cursor->next());
        _cursor->set_next(newA);
        _size++;
    }
    //
...
void SList<T>::remove()
{
    ...
    // Case 2: remove in a middle position.
        ...
        auto ptr = _head;
        while(ptr->next() != _cursor){
            ptr = ptr->next();
        }
        ptr->set_next(_cursor->next());
        _cursor = ptr->next();
    }
...
bool SList<T>::find(T const &it)
{
    ...
    _cursor = _head;
    auto prev = _head;
    while(_cursor != nullptr and _cursor->item() != it){
        prev = _cursor;
        _cursor = _cursor->next();
    }
    if(_cursor != nullptr){
