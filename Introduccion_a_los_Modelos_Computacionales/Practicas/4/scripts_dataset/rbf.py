...
@click.option('--ratio_rbf', '-r', default=0.1, required=False,
              help=u'Ratio of neurons (RBF) for train patterns.')
...
@click.option('--classification', '-c', is_flag=True,
              help=u'Classification or Regression. ')
...
def train_rbf(train_file, test_file, classification, ratio_rbf, l2, eta, outputs, model_file=""):
    ...
    if not classification:
        ...
        test_b_outputs = log_b.fit_transform(test_outputs).toarray()
        train_b_outputs = log_b.fit_transform(train_outputs).toarray()   

        
        train_mse = mean_squared_error(train_b_outputs, logreg.predict_proba(r_matrix))
        test_mse = mean_squared_error(test_b_outputs, logreg.predict_proba(r_matrix_test))
...
def init_centroids_classification(train_inputs, train_outputs, num_rbf):
    """ Initialize the centroids for the case of classification
        ...
        train_outputs: array, shape (n_patterns,n_outputs)a
    ...
    """
    centroids, test_x, train_y, test_y = train_test_split(train_inputs, train_outputs, train_size = num_rbf, stratify = train_outputs)
